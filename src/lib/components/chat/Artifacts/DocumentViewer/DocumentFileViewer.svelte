<script lang="ts">
	...
	const readPyodideFile = (signal: AbortSignal, maximumBytes = maxBytes): Promise<ArrayBuffer> => {
		if (fileId) {
			return readSavedFile(fileId, maximumBytes, signal);
		}
	...
	const refreshPyodideFile = (event: Event) => {
		...
		const action = getWorkspaceFileRefreshAction(changedPaths, path);
	...
	const handleWorkspaceFileUpdate = (
		...
		if (!update) return;
		const action = getWorkspaceFileUpdateAction(update, path);
