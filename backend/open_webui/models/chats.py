...
class ChatTable:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if use_compare_and_swap:
                        ...
                        # Claim the JSON version before flushing related message/note rows.
    ...
    async def mutate_message_by_id(
        self, chat_id: str, message_id: str, mutator: Callable[[dict], Any], *, user_id: str
    ) -> tuple[ChatModel, Any] | None:
        """Claim paused interactions atomically in both native message stores."""
        async def mutate(chat_data, session):
            history = chat_data.setdefault('history', {})
            message = copy.deepcopy(history.get('messages', {}).get(message_id) or {})
                ...
                for column in ChatMessage.__table__.columns:
                    if column.key not in ChatMessages.EXCLUDED_COLUMNS:
                        ...
                        if value is not None:
                            message[ChatMessages.DB_TO_JSON_KEY_MAP.get(column.key, column.key)] = copy.deepcopy(value)
            if not message:
                raise ValueError('Message not found.')
            result = mutator(message)
            message = self._clean_null_bytes(message)
            self.upsert_message_to_history(history, message_id, message)
            now = int(time.time())
            if row is None:
                session.add(ChatMessages._build_message(f'{chat_id}-{message_id}', chat_id, user_id, message, now))
            else:
                ChatMessages._apply_message_data(row, message, now)
            return chat_data, result

        return await self.mutate_chat_by_id(chat_id, mutate, user_id=user_id)
