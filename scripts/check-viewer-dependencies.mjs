...
export function checkViewerDependencies(projectRoot = root) {
	...
	for (const [name, expectedLicense] of Object.entries(VIEWER_LICENSES)) {
		...
		if (lockEntry.license !== expectedLicense) {
			errors.push(`${name}: package-lock license ${lockEntry.license ?? 'missing'} != ${expectedLicense}`);
		}
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
	...
	if (errors.length) {
		...
		console.log(`viewer dependency/license check passed (${Object.keys(VIEWER_LICENSES).length} packages)`);
	}
}
