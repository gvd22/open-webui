...
export function checkBuildSourceMaps(buildRoot, ceilings = RENDERER_CHUNK_CEILINGS) {
	...
	for (const worker of filesUnder(join(buildRoot, '_app/immutable/assets'), '.mjs')) {
		if (worker.split('/').pop()?.startsWith('pdf.worker.')) packageFiles.get('pdfjs-dist')?.add(worker);
	}
	...
	for (const [packageName, ceiling] of Object.entries(ceilings)) {
		...
		for (const file of files) {
			...
			if (size > ceiling) errors.push(`${packageName}: ${file} is ${size} bytes; ceiling is ${ceiling}`);
		}
	}
	return errors;
}
	...
	try {
		...
		if (process.argv.includes('--print-baseline')) {
			for (const [packageName, ceiling] of Object.entries(RENDERER_CHUNK_CEILINGS)) {
				...
				if (packageName === 'pdfjs-dist') matching.push(...filesUnder(join(buildRoot, '_app/immutable/assets'), '.mjs').filter((file) => file.split('/').pop()?.startsWith('pdf.worker.')));
				for (const file of new Set(matching)) console.log(`${packageName}: ${statSync(file).size} bytes (ceiling ${ceiling})`);
			}
		}
		if (errors.length) {
