...
test('requires source-map-backed renderer chunks instead of trusting a manifest', async () => {
	...
	for (const packageName of ['pdfjs-dist', 'docx-preview', '@aiden0z/pptx-renderer']) {
		assert.equal(typeof checker.RENDERER_CHUNK_CEILINGS[packageName], 'number');
	}

	const fixtureDir = join(new URL('../', import.meta.url).pathname, `.viewer-bundle-${process.pid}`);
	await mkdir(join(fixtureDir, 'build/_app/immutable/chunks'), { recursive: true });
	await writeFile(join(fixtureDir, 'build/manifest.json'), JSON.stringify({ stale: 'pdfjs-dist docx-preview pptx-renderer' }));
	...
	for (const [name, source] of Object.entries({
		...
		'@aiden0z/pptx-renderer': 'node_modules/@aiden0z/pptx-renderer/dist/index.js'
	})) {
		const stem = name === '@aiden0z/pptx-renderer' ? 'pptx-renderer' : name;
...
test('rejects a renderer chunk over its measured ceiling', async () => {
	const checker = await import(new URL('./check-viewer-bundle.mjs', import.meta.url));
	const fixtureDir = join(new URL('../', import.meta.url).pathname, `.viewer-bundle-over-${process.pid}`);
	await mkdir(join(fixtureDir, 'build/_app/immutable/chunks'), { recursive: true });
	...
	await writeFile(
		...
		JSON.stringify({ version: 3, file: 'pdfjs-dist.js', sources: ['node_modules/pdfjs-dist/index.js'] })
	);
