<script lang="ts">
	...
	let workspaceContents: WorkspaceContent[] = workspaceLru
		? []
		: workspacePanels
			? [buildWorkspaceFileContent(documents.pdf.path), buildWorkspaceFileContent(documents.docx.path)]
...
<main class:dark={new URLSearchParams(window.location.search).get('theme') === 'dark'}>
	...
	{#if unsupportedPath || workspacePanels}
		...
		>
			...
			{:else}
				Files keeps unsupported formats in its existing preview; supported files open this tab panel.
			{/if}
		</div>
	{/if}
	{#if workspaceLru}
	...
	<div class="workspace">
		<WorkspaceDocumentPanels
			...
			{selectedContentId}
			runtime={{ kind: 'terminal', terminalId: 'viewer-runtime', files: true, writable: true, shell: true, ports: true }}
...
<style>
	:global(html, body, #app) { margin: 0; height: 100%; font-family: system-ui, sans-serif; }
	main { height: 100%; background: #f5f4f1; color: #202020; }
	main.dark { background: #171719; color: #f4f4f5; }
	header { height: 40px; display: flex; align-items: center; justify-content: space-between; padding: 0 12px; }
	button { font: inherit; }
	.files-state { padding: 0 12px 8px; font-size: 12px; color: #5f5f5f; }
	.lru-controls { display: flex; flex-wrap: wrap; gap: 4px; padding: 0 12px 8px; }
	.lru-controls button { border: 1px solid #bbb; border-radius: 4px; padding: 2px 6px; }
	output { display: block; padding: 0 12px 4px; font-size: 11px; }
	.workspace { position: relative; height: calc(100% - 40px); min-height: 0; overflow: hidden; }
</style>
