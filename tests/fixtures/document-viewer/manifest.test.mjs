...
test('manifest describes exactly the three deterministic sanitized fixtures', async () => {
	...
	for (const fixture of manifest.fixtures) {
		...
		assert.equal(typeof fixture.expectedPages === 'number' || typeof fixture.expectedSlides === 'number', true);
		...
		if (fixture.format === 'pdf') {
			...
			assert.ok(file.includes(Buffer.from(fixture.sentinel)), `${fixture.path} contains its sentinel`);
		} else {
			const entry = fixture.format === 'docx' ? 'word/document.xml' : 'ppt/slides/slide1.xml';
			const extracted = await execFileAsync('unzip', ['-p', fixturePath, entry], { encoding: 'utf8' });
